...
const Grant = () => {
  ...
  const handleDelete = (id) => {
    ...
    return (
        ...
        <ul>
        {posts
  .filter(post => post.access === false) // Filter out posts where access is false
  .map((post, index) => (
    <li key={index} className={`relative bg-gradient-to-r from-violet-500 to-fuchsia-500 transform transition duration-300 bg-gray-100 p-3 rounded-lg mb-2 hover:scale-105`}>
